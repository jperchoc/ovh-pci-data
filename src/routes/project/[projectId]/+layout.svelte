<script lang="ts">

    import type { NavItemWithChildren, SidebarNavItem } from "$lib/types/nav";
	import { page } from "$app/stores";
	import DocsSidebarNav from "$components/base/nav/DocsSidebarNav.svelte";

    const nav:SidebarNavItem[] = [
        {
            title: 'Storage',
            items: [{
                title: 'Databases',
                href: `/project/${$page.params.projectId}/database`
            }] as NavItemWithChildren[]
        },
        {
            title: 'Dataprocessing',
            items: [
                {
                    title: 'Jobs',
                    href: `/project/${$page.params.projectId}/dataprocessing/job`
                },
                {
                    title: 'Notebooks',
                    href: `/project/${$page.params.projectId}/dataprocessing/notebook`
                }
            ]as NavItemWithChildren[]
        },
        {
            title: 'AI',
            items: [
                {
                    title: 'Notebooks',
                    href: `/project/${$page.params.projectId}/ai/notebook`
                },
                {
                    title: 'Jobs',
                    href: `/project/${$page.params.projectId}/ai/training`
                },
                {
                    title: 'Deploy',
                    href: `/project/${$page.params.projectId}/ai/deploy`
                }
            ]as NavItemWithChildren[]
        }
    ]

</script>
<!-- <header>
    <nav>
        <ul>
            {#each nav as link}
                <li>
                    <a href={link.href} class:active={$page.url.pathname.startsWith(link.href)}>{link.label}</a>
                </li>
            {/each}
        </ul>
    </nav>
</header> -->

<div
	class="bg-secondary max-w-full flex-1 items-start md:grid md:grid-cols-[220px_minmax(0,1fr)] md:gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10"
>
	<aside
		class="bg-background fixed top-14 z-30 -ml-2 hidden h-[calc(100vh-3.5rem)] w-full shrink-0 overflow-y-auto border-r md:sticky md:block container"
	>
		<div class="py-6 pr-6 lg:py-8">
			<DocsSidebarNav items={nav} />
		</div>
	</aside>
    <main class="relative py-6 lg:gap-10 lg:py-8 xl:grid xl:grid-cols-[1fr]">
        <div class="mx-auto w-full min-w-0">
            <slot />
        </div>
    </main>
</div>