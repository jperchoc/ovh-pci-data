<script lang="ts">
	...
	import Actions from '$components/pages/ai-dashboard/table/Actions.svelte';
	import {
		DataTable,
		TableDateCell,
		TableLinkCell,
	...
	const columns = table.createColumns([
		...
		table.column({
			header: '',
			accessor: (registries) => registries,
			cell: ({ value }) => {
				return createRender(Actions).on('delete-clicked', () =>
					dispatch('delete-clicked', { registries: value })
				);
			}
		}),
	]);
